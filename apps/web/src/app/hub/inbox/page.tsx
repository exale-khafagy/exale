...
export default function InboxContactPage() {
  ...
  return (
    <div>
      ...
      {/* Submission list (same card layout on all screen sizes so desktop and mobile stay in sync) */}
      {filteredSubmissions.length > 0 && (
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <dd className="space-y-1 text-sm">
                    ...
                    {matchingProfile.title && (
                      <p className="text-gray-900 dark:text-white"><span className="text-gray-500 dark:text-gray-400">Title:</span> {matchingProfile.title}</p>
                    )}
                    {Array.isArray(matchingProfile.socialChannels) &&
                      matchingProfile.socialChannels.map((ch, i) => (
                        <a key={i} href={ch.url} target="_blank" rel="noopener noreferrer" className="text-royal-violet hover:underline block">{ch.platform}</a>
                      ))}
                  </dd>
                </div>
              )}
